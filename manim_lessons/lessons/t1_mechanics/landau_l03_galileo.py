...
class GalileoBase(Scene):
 LANGUAGE="zh-TW"
 def setup(self):
  apply_global_config();self.data=LOCALES[self.LANGUAGE];self.s=self.data["screen"];self.clock=ValueTracker(0);self.audio_dir=pathlib.Path(__file__).resolve().parents[2]/"samples"/"audio_l03"/self.LANGUAGE
