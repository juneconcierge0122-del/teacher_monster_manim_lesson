...
60: {"zh": ('第 4 章：等度連續與 Cauchy 序列', [
 ...
 'Section 6 is short and introduces equicontinuity. A family of functions is equicontinuous at a point if each is continuous there and, given epsilon, one delta serves the whole family. Note the direction: one delta for many functions, where uniform continuity was one delta for many points.',
 ...
 'Each such function collects the members of the family within a quarter epsilon of it on that finite set. Two things need proving: the collections cover the family, and each has diameter at most epsilon. The book then remarks that the argument is completely elementary and hard, and that sophisticated and difficult are by no means equivalent.',
 'Section 7 is completeness. If a sequence converges its terms plainly get close to one another; conversely a sequence whose terms get close together ought to converge. The definition: for every epsilon there is an N beyond which any two terms are within epsilon. Such a sequence is Cauchy.',
 ...
 'Lemma 7.3: Lipschitz maps carry Cauchy sequences to Cauchy sequences, bounded linear maps being a special case, and Theorem 7.1 extends this to uniformly continuous maps. Continuity alone will not do: one over x carries the Cauchy sequence one over n to the sequence n, which is not Cauchy.',
 'The definition: a metric space is complete if every Cauchy sequence in it converges to a point of it. A complete normed space is called a Banach space. Theorem 7.2 says the line is complete, by noting that a Cauchy sequence is bounded and so has a convergent subsequence, then quoting Lemma 7.2.',
 'Theorem 7.3 says completeness travels: if the domain is complete and the map is a continuous bijection whose inverse is Lipschitz, the image is complete too. In particular an invertible bounded linear map carries a Banach space to one, and equivalent norms are complete together or not at all.',
 'Theorem 7.4 says a product of two Banach spaces is one, and this extends to finite products. The second corollary matters: every finite-dimensional vector space, under any norm at all, is a Banach space. The line is complete, products preserve it, and in finite dimensions all norms are equivalent.',
])},
}
...
60: {
 ...
 4: '𝔉 ᵍ  =  { f  :  ρ ( f p , g p ) < ϵ / 4  ,  p ∈ D }          diam  ≤  ϵ',
 5: 'm , n  >  N            ⇒            ρ ( x ₘ , x ₙ )    <    ϵ',
 6: '{ x ₙ }   →   a        ⇒        { x ₙ }   Cauchy',
 7: 'ρ ( T x , T y )  ≤  C ρ ( x , y )        ⇒        Cauchy  ↦  Cauchy',
 8: 'A   complete            V   Banach',
 9: 'p  ≈  q          ⇒          ( ⟨ V , p ⟩  complete  ⇔  ⟨ V , q ⟩  complete )',
 10: 'dim V  <  ∞            ⇒            V   Banach',
},
}
