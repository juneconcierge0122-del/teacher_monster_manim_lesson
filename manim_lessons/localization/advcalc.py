...
58: {"zh": ('第 4 章：序列緊緻性', [
 ...
 '有一個看起來很怪的引理。如果 x_n 不收斂到 a，那就存在一個 ε，使得可以挑出一整支子序列，每一項都離 a 至少 ε 遠——而那一支的任何子序列也都收斂不到 a。反過來寫就是引理 4.1。',
 ...
 '定理 4.1 說連續映射把緊緻集送成緊緻集。而實數線上非空的緊緻集含著最大與最小元——因為上確界是集合裡某個序列的極限，而集合是閉的。合起來就是那條熟悉的推論：連續函數在緊緻定義域上取得到最大與最小值。',
 ...
 '峰項有無窮多個的話，它們本身就是一支遞減的子序列；只有有限多個的話，最後一個之後每一項都還有更大的在後面，於是挑得出嚴格遞增的一支。兩條合起來就是定理 4.3：有界實數列必有收斂子序列。',
 '定理 4.4 用歸納推到 ℝⁿ：把它看成 ℝ 的 n 減一次方乘上 ℝ，先對前面那一塊用歸納假設取子序列，再對最後一個座標取一次。定理 4.5 隨即得到：ℝⁿ 的有界閉集是序列緊緻的。',
 '最後補上第 3 章欠的那個洞。定理 4.6：ℝⁿ 上所有範數等價。一個方向是三角不等式直接給的；另一個方向是把任意範數限制到一範數的單位球面上，那是緊緻集，所以最小值取得到而且不是零。',
