...
class Narrator:
    ...
    def _wrap(self, text: str) -> str:
        ...
        if len(text) <= self.max_chars:
            return text
        # Space-delimited languages should wrap between words, never through
        # a word. CJK copy falls back to the character-based path below.
        if " " in text and sum(ch.isascii() for ch in text) / len(text) > 0.7:
            return "\n".join(textwrap.wrap(text, width=self.max_chars))
        # 中文情境: 直接按字數切
        lines = []
        i = 0
        while i < len(text):
