...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E76 已上傳（第 6 章 §1、§2 完結） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E77，§3「線性方程」**（書頁 276–280，習題 3.1–3.9 在 280–281）：
定理 3.1（解空間是 S 的核、右反元素）、定理 3.2（基本解 K ₜ）、定理 3.3（非齊次的公式）、
定理 3.4（常係數的 e ᵗᵀ 與 Jordan 型展開）。**§3 是全章第二長的一節，細目表給它 1 集，
但四個定理擺一集可能太滿——切不下就拿 2 集，並與 §4 或 §7 調整，OUTLINE 那一欄要一起改。**
