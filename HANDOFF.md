...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E68 已上傳（第 0 到 4 章完結，第 5 章做到 §1 完） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E69**，接第 5 章 §2「正交投影」（書頁 252–256，習題 256–257）。
那一節是全章最長的一節，**細目表給它 2 集（E69–E70）**：引理 2.1（最佳近似 ⇔ 差向量垂直子空間）、
引理 2.2 與定理 2.1（完備子空間的正交分解 V = M ⊕ M⊥）、正交投影就是最佳近似、
引理 2.3 與 2.4（投影的可加性、到一維子空間的投影）、Fourier 係數、正交規範集合，
最後是定理 2.2 Bessel 不等式。**動工前一樣要自己核頁碼與內容／習題的分界。**
