...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E74 已上傳（第 6 章已開） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E75，§1「基本定理」的後半**（書頁 269–272）：整體解與極大解、
定理 1.3、定理 1.4（Lipschitz 界隨 t 連續時解活在整個 I 上），以及把 n 階方程化成一階系統的定理 1.5。
E74 收在定理 1.2。**第 6 章的細目表已經補進 `OUTLINE.md`**（2026-09-16），七節都核過了，
所以這一章不必再逐節重核頁碼，但每一集動工前仍然要自己翻一次要講的那幾頁。
