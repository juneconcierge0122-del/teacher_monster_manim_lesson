...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E70 已上傳（第 0 到 4 章完結，第 5 章做到 §2 完） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E71**，接第 5 章 §3「自伴變換」（書頁 257–260，習題 260–261）。
細目表給它 1 集。動工前照例自己核一次頁碼與內容／習題的分界。
