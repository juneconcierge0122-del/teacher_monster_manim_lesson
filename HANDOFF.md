...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E73 已上傳（第 0 到 5 章完結） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E74，也是第 6 章「微分方程」的第一集**（從書頁 266 起，§1「基本定理」）。
第 5 章已完結。**第 6 章還沒有細目表**，動工前要先照書一頁一頁核出各節的內容頁與習題頁，
補進 `OUTLINE.md`——第 5 章那張是這樣核出來的，五節全準，是全書唯一一整章沒錯的。
