...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E72 已上傳（第 0 到 4 章完結，第 5 章做到 §4 完） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E73**，接第 5 章 §5「緊變換」（書頁 264–265，**整節沒有習題**，
265 講完直接進第 6 章）。這是第 5 章的最後一節，做完就進第 6 章「微分方程」。
動工前照例自己核一次頁碼——第 5 章那張細目表是自己核過的，§1 到 §4 四節都對上了。
