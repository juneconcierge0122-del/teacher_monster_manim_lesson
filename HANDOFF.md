...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E67 已上傳（第 0 到 4 章完結，下一集開第 5 章） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E68**，第 4 章已經完結（E67 收在加星號的 §12「弱方法」，書頁 247），
接下來開第 5 章「純量積空間」第 1 節。章首的引言與 §1 都從書頁 247 起（OUTLINE 寫 248）：
純量積的三條（對第一變數線性、對稱、正定）、半純量積、兩個例子（ℝⁿ 上的 Σxᵢyᵢ 與
C([a,b]) 上的 ∫fg）、複空間要改成 Hermitian 對稱（**書上明說只做實數情形**）、
定理 1.1 Schwarz 不等式（證法是「那個 t 的二次式不變號」）、推論「純量積範數真的是範數」，
以及 Hilbert 空間的定義。**動工前一樣要自己核 §1 的內容收在哪一頁、習題從哪裡開始。**

**OUTLINE 的集數仍然是估的，動工前先照書核一次**——那張表的「書頁」欄**已經錯過九次**
（見 advcalc 的 STATUS.md）：前八次是把各節的習題頁算了進去，第九次（§12）是把下一章的
開頭算了進去。第 4 章的細目表在 OUTLINE.md（2026-08-30 補），集號 E56–E67 與它一致。
