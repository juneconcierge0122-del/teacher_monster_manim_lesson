...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E80 已上傳（第 6 章 §1–§4 完結） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E81，§5「解非齊次方程」**（書頁 288–292，習題 5.1–5.22 在 293–294）：
先是常係數而 g 本身落在某個常係數算子零化空間裡的那個實用解法（試 c e ᵇᵗ、
L 與 S 有公因式時要多帶一個 t ⁱ），再把定理 3.3 翻譯成矩陣語言的一般方法。
**細目表給 §5 一集。** 動工前仍然自己翻一次那幾頁——§4 那次核起來與表完全相符，
但那是那張表第一次在動工前核就準，不要因此就省掉這一步。
**第 6 章的細目表已經補進 `OUTLINE.md`**（2026-09-16），七節都核過了，所以這一章不必再逐節
重核頁碼，但每一集動工前仍然要自己翻一次要講的那幾頁。
**§4 那次核下來與表完全相符**（內容 281–287、習題在 287、§5 從 288 起），是那張表第一次
在動工前核就準；但切點不要照注記照抄——原本寫「定理 4.1 之後」，實際那樣會讓前半只有兩頁。

**畫面上用到罕用字元，要在 1080p 下放大看一次。** E80 把三撇 `‴` 畫成導數鏈的第四個方塊，
而那個字元字型裡沒有、被畫成兩撇，於是兩個方塊變成同一個函數——
bounds 與 collide 都看不到（它在框內、沒疊到東西），480p 的 probe 幀也分不出來。
細節與另外兩個查過沒問題的字元（`λ̄`、`ᐟ`）記在 advcalc 的 STATUS.md。

**一輪可以做兩集。** `PARALLEL=1 SCENE_PREFIX=AdvCalcE bash tools/queue.sh 77:a.py 78:b.py`
四支場景同時渲染，實測 48 分鐘（序列要約 3.2 小時），代價只有約 17% 的單支速度。
**但看幀仍然一集一集過**——E75/E76 那一輪 probe 幀抓到四處，兩處是畫面在說錯話，
22 張幀一次看完和 11 張看兩次抓到的東西不一樣。細節見 `docs/PLAYBOOK.md` 第 4 節。

**第 5 章的細目表已經補進 `OUTLINE.md`**（2026-09-12，E68 那一輪）：內容頁與習題頁分開標，
並記下 **§5「緊算子」整節沒有習題**。那張表的「書頁」欄在第 4 章以前**錯過九次**
（見 advcalc 的 STATUS.md），都是把習題頁或下一章的開頭算了進去，所以第 5 章這張是自己核的。

**還有一筆未決的帳：langscan 加進來（E49）之前的集數從來沒掃過。**
2026-09-01 補掃 E00–E60，共 82 處，集中在 E00–E20。絕大多數是英文單字寫進 `FORMULAS`，
所以中文版影片的公式列上有英文；另有一處相反（E32 的「小」出現在英文版）。
細節與清單在 advcalc 的 STATUS.md。這些都已上傳，要修就得重渲染加重新上傳、
舊版還得人工去 YouTube Studio 刪，所以**先問過使用者再動**。
