...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E60 已上傳（第 0 到 3 章完結，第 4 章做到第 7 節前半） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E61**，接第 7 節後半：定理 7.5 到 7.11（書頁 218–221）。
內容是 𝔅(A, W) 與 Hom(V, W) 都是 Banach 空間、完備集的閉子集、
𝔅𝒞 是閉子空間（那張 ε/3「up, over, and down」的圖是書上的 Fig. 4.3，**要自己重畫**）、
「均勻收斂的連續函數列，極限也連續」、定理 7.9、引理 7.4、定理 7.10（緊緻 ⇔ 全有界且完備），
最後是無窮級數與定理 7.11（Banach 空間裡絕對收斂就收斂）與 Weierstrass 判別法。
**§7 的內容收在書頁 221**，221–223 是習題 7.1–7.21，依第 8 節不做解答。

**OUTLINE 的集數仍然是估的，動工前先照書核一次**——那張表的頁碼**已經錯過五次**
（見 advcalc 的 STATUS.md）：它的「書頁」欄含各節的習題頁，實際內容比表上少。
最新一次是 §7 寫成 216–223，實際內容只到 221。第 4 章的細目表在 OUTLINE.md
（2026-08-30 補），到 E60 為止集號都與它一致。
