...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E38 已上傳（第 0、1、2 章完結，第 3 章做到第 6 節完） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E39**，接第 3 章第 7 節「方向導數與均值定理」（書頁 146 起）。
E38 已經把 §6 收完上傳了（微分的定義、定理 6.1 的五條規則、定理 6.2 鏈鎖規則；
實際收在書頁 144，145–146 是習題 6.1–6.18，依第 8 節不做解答）。
到 E38 為止，實際集數都與 OUTLINE 的預估相符。
