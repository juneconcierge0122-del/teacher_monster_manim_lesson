...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E69 已上傳（第 0 到 4 章完結，第 5 章做到 §2 前半） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E70**，接第 5 章 §2 的後半（書頁 254–256，習題 256–257）：
正交規範集合、定理 2.2（Bessel 不等式、Fourier 級數的收斂與 Parseval 條件）、
基底的定義與定理 2.3（正交規範序列是基底 ⇔ 線性包稠密）及其推論、
「正交基的係數與其他基向量無關」那段評註、引理 2.5 Gram–Schmidt
（書上算了 {xⁿ} 在 C([0,1]) 的前三項：1、x − ½、x² − x + ⅙），
最後是定理 2.4：θ（β ↦ (·, β)）是同構 ⇔ V 是 Hilbert 空間。
