...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E71 已上傳（第 0 到 4 章完結，第 5 章做到 §3 完） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E72**，接第 5 章 §4「正交變換」（從書頁 262 起）。
動工前照例自己核一次頁碼與內容／習題的分界——第 5 章那張細目表是自己核過的，
§1、§2、§3 三節都對上了，可是仍然要核。
