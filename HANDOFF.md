...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E59 已上傳（第 0 到 3 章完結，第 4 章做到第 5 節完） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E60**，接第 4 章第 6 節「等度連續」（書頁 215–216）與第 7 節「完備性」
（216 起）。**§6 只有一頁多，單獨成一集會太空，所以照 OUTLINE 併進 E60**——
第 4 章裡另一處合併是 §2 併進 E57。E60 的旁白與公式已經寫好放在
`TOPICS_ADVCALC[60]` / `FORMULAS_ADVCALC[60]`（隨 E59 一起提交），但**場景檔還沒開**，
而且 `langscan advcalc 60` 目前是 7 個 leak：`全有界`（CJK）、`diam`、`complete`，
以及 `Cauchy`、`Banach` 兩個專有名詞。前三個要改寫成純符號或移到 `MODE_LABEL`；
後兩個在中英文旁白裡都是原樣寫的（「Cauchy 序列」「Banach 空間」），
所以更可能該進 `langscan.py` 的 `WHITELIST`——**動工時一起決定，不要各修一半。**

**OUTLINE 的集數仍然是估的，動工前先照書核一次**——那張表的頁碼錯過四次（見 advcalc 的 STATUS.md）：
它的「書頁」欄含各節的習題頁，實際內容比表上少。第 4 章的細目表在 OUTLINE.md
（2026-08-30 補），到 E59 為止集號都與它一致。
