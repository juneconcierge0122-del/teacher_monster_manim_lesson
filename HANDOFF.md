...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E78 已上傳（第 6 章 §1–§3 完結） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E79，§4「n 階線性方程」**（書頁 281–287，習題 4.1–4.19 在 287）：
把 n 階方程化成一階系統之後，定理 4.1 重述線性理論、然後是降階法、常係數的 p ( D )、
定理 4.2（( t − b ) ⁿ 的基底 e ᵇᵗ、t e ᵇᵗ、…）與定理 4.3（複根時的 cos / sin 形式）。
**細目表給 §4 兩集，這次估得應該比較準**——§3 就是估 1 集而實際要 2 集。
切點建議在定理 4.1 之後：前半是把 §3 的結構搬到 n 階，後半是常係數的顯式解法。
