...
| `series/advcalc/` | Loomis & Sternberg《Advanced Calculus》 | **進行中**：E00–E43 已上傳（第 0、1、2 章完結，第 3 章做到第 9 節完） |

**現在的方向是 `advcalc`。**《力學》已完結；《場論》仍是空殼。
接手時先讀 `series/advcalc/STATUS.md`（進度、連結、踩過的坑）與
`series/advcalc/OUTLINE.md`（全書解析與 169 集規劃）。

**眼前第一件事是 E44**，接第 3 章第 10 節「初等應用」（書頁 161 起）。
E43 已經把 §9 收完上傳了（定理 9.1–9.4、雅可比矩陣、矩陣形式的鏈鎖規則、雅可比行列式；
實際收在書頁 159，159–160 是習題 9.1–9.11，依第 8 節不做解答）。
到 E43 為止，實際集數都與 OUTLINE 的預估相符。
