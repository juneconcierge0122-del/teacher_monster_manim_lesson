...
class LessonArchetype(Scene):
    ...
    def teach_concept(self, term: str, definition: str,
        ...
        若 PERSONA 已設, 會自動:
          ...
          - 加總到 jargon 計數
        """
        if getattr(self, "_last_teach_block", None) is not None:
            self.play(FadeOut(self._last_teach_block), run_time=0.35)
            self._last_teach_block = None

        if self._heading_group is not None:
            self.play(FadeOut(self._heading_group), run_time=0.3)
            self._heading_group = None
        ...
        defn_mob.scale_to_fit_width(min(defn_mob.width, config.frame_width - 1.5))
        ...
        # 3. 例子
        if example:
            ex = Text(f"e.g.：{example}", font_size=FS_BODY, color=DIM)
